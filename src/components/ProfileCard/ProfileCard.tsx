  ...
  const onSaveEditUser = async (userToUpdate: User) => {
    if (userToUpdate._id === user?._id) {
      ...
      const traineeIdx = trainees.findIndex(
        (trainee) => trainee._id === userToUpdate._id
      )
    ...
    <>
      <Card
        ...
        className={`card user-details ${prefs.isDarkMode ? 'dark-mode' : ''} ${
          traineeUser?._id === userToDisplay?._id ? 'selected' : ''
        } ${prefs.favoriteColor}`}
      >
        ...
        >
          ...
          <div className='profile-info'>
            ...
            <Typography variant='body1'>
              {userToEdit?.details?.height || 0} {t('user.cm')}
            </Typography>
            <Typography variant='body1'>
              {new Date(userToEdit?.details?.birthdate || 0).toLocaleDateString(
                'he'
              )}
            </Typography>
          </div>
